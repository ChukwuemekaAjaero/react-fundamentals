...
It's actually pretty intersting how you can use different methods to render the
same thing. Take this first exercise as an example. In the same way I can add
objects like an event listener with `document.addEventListener`, that's the same
way I can add nodes to the DOM tree.

> ❗ Remember to always create a reference for the nodes you are creating.

💭 It seems that the returned object's type the call `document.createElement`
and `document.getElementById()` actually has some properties that can be
accessed.

> The returned type from `document.createElement` is an `HTMLElement`
