...
The exercise basically wants you to create DOM elements, but using the React API
instead of the browser API.

Like with the first exercise, you've got to think about the properties of the
DOM node that you want to create. With this exercise, there's not much point in
using React, but this is a good way to see how React does things under the hood.

The **React** package, most often seen when using npm, is used to create the
ReactElements, while ReactDOM is the platform-specific package that React code
is rendered too. You need to have the platform specific packages as React is
used on the browser, but also on native.

#### Nesting DOM Elements

If you want have multiple nodes in the DOM, you nee to nest them somehow. This
can be done by having other ReactElements be the children of another
ReactElement. The `children` property of a ReactElement can be a single data
type, or an array.
